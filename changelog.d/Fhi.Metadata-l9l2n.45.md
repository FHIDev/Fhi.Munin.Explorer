...
- **A saved list's "Sist endret" no longer moves when variables are added or removed.** Munin
  ...
  `VariableList.UpdatedAt` off `VariableListState.Lists` now sees the same value the API holds; the
  variable count beside it still moves on both. (Fhi.Metadata-l9l2n.45)
