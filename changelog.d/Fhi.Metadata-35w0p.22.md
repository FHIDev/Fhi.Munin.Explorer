...
- **`KildeView` draws its sections in the order the API places them, property sections and built-in
  ...
  so reordering the page, renaming a section or moving a property between sections is an edit a
  Munin curator makes rather than a release of this package. Each placed property group becomes a
  section of its own, anchored at the catalogue's group key and listed in the contents nav.
