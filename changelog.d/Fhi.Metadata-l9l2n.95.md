...
- **Saved variable lists show two coverage columns, Kodeverk and Statistikk** - `VariableListView`
  ...
  ("Yes"/"No" in English), never as a glyph. The headings are Kodeverk and Statistikk, and Kodeverk and
  Statistics in English, as in Kelda's share columns. The values come from `hasKodeverk` and `hasStatistikk`, which
