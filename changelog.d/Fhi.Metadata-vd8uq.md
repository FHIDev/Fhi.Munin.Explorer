...
- **The sample hosts rendered every unstyled string at 16px, where helsedata.no's inherited base
  ...
  size read 2px small on an ordinary desktop and 5px small above 2881px. `html` stays 16px on
  both sides, so `rem` values are unaffected — only the inherited size moves. A host reading the
