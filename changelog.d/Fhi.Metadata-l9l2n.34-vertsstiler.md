...
- **`button-square--ghost-blue` is a new name to style**, and it is Stiler's own rather than one
  this package invented. A host on Stiler already has it; a host with a stylesheet of its own gets
  the drilldown's way back rendered at browser defaults until it adds a rule. Both sample
