...
- **Stiler does carry a breadcrumb rule, and the detail pages' trail wears it; the two search
  ...
  `__last-crumb` — as global unscoped classes, and `DetailTrail` already wears them. The real
  reason those two trails stay unclassed is that their steps do not navigate: the hierarchy trail's
  narrow the search, and the kilde trail's one button discloses the kilde in place, so the
  breadcrumb vocabulary does not describe either. No markup changed: both trails emit the same
