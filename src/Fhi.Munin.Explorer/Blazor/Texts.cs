...
internal sealed record Texts(
    ...
    // place, so it counts them and the list beside the trail names every one. Its own text rather
    ...
    Func<string, string> StatisticsFromOlderYearSet,
    ...
    string ConfirmDeleteNo,
    string ListActionError,
    // Shared by sharing, copying and emptying: the reason an empty list refuses them, and why a
    // name cannot be given to a new list.
    string ListIsEmpty,
    string ListNameRequired,
    string ListNameTooLong,
    string ListNameTaken,
    // Sharing a list as a code, and opening one somebody shared (Fhi.Metadata-ntpbd.1). The code
    // is readable by anyone who has it for 90 days, which is why the validity is said beside it.
    string ShareList,
    string ShareCodeLabel,
    string ShareLinkLabel,
    string ShareByEmail,
    // (list name) — the e-mail's subject line.
    Func<string, string> ShareMailSubject,
    // (code, link or null) — the e-mail's body.
    Func<string, string?, string> ShareMailBody,
    string ShareValidity,
    string ShareError,
    string OpenSharedList,
    string SharedListCodeLabel,
    string OpenSharedListSubmit,
    string SharedListEyebrow,
    string SaveSharedList,
    string SaveSharedListName,
    string SaveSharedListSubmit,
    string CloseSharedList,
    // Text, not a login control, for the reason SignInForVariableLists gives.
    string SignInToSaveSharedList,
    // (code) — an unknown, expired or malformed code.
    Func<string, string> SharedListNotFound,
    string SharedListError,
    // Copying a list and emptying one (Fhi.Metadata-ntpbd.2). A copy carries the variables and not
    // the annotations, which the copy's own disclosure says; emptying is confirmed as deleting is.
    string CopyList,
    string CopyListName,
    string CopyListSubmit,
    // (list name) — what the copy's name field starts as.
    Func<string, string> DefaultCopyName,
    string CopyListLeavesDesiredData,
    string CopyIncomplete,
    // (copy's name) — the same, when the reader has moved to another list and the copy is not below.
    Func<string, string> CopyIncompleteElsewhere,
    string EmptyListAction,
    string ConfirmEmptyList,
    string ConfirmEmptyYes,
    string ConfirmEmptyNo,
    ...
    string SignInForVariableLists)
{
    ...
    private static readonly Texts No = new(
        ...
        StatisticsFromOlderYearSet: yearSet => $"Statistikken er fra årssett {yearSet}, det siste som har statistikk registrert.",
        ...
        ConfirmDeleteNo: "Avbryt",
        ListActionError: "Kunne ikke endre listen nå. Prøv igjen om litt.",
        ShareList: "Del liste",
        ListIsEmpty: "Listen er tom",
        ShareCodeLabel: "Delekode",
        ShareLinkLabel: "Lenke til listen",
        ShareByEmail: "Send koden på e-post",
        ShareMailSubject: name => $"Delt variabelliste: {name}",
        ShareMailBody: (code, link) => link is null
            ? $"Åpne listen med delekoden {code}. Koden virker i 90 dager."
            : $"Åpne listen med delekoden {code}, eller med denne lenken: {link} Koden virker i 90 dager.",
        ShareValidity: "Koden virker i 90 dager.",
        ShareError: "Kunne ikke dele listen nå. Prøv igjen om litt.",
        OpenSharedList: "Åpne delt liste",
        SharedListCodeLabel: "Delekode for listen du vil åpne",
        OpenSharedListSubmit: "Åpne listen",
        SharedListEyebrow: "Delt liste",
        SaveSharedList: "Lagre som min liste",
        SaveSharedListName: "Navn på din kopi av listen",
        SaveSharedListSubmit: "Lagre listen",
        CloseSharedList: "Lukk delt liste",
        SignInToSaveSharedList: "Logg inn for å lagre listen som din egen.",
        SharedListNotFound: code => $"Fant ingen delt liste med koden {code}. En kode virker i 90 dager.",
        SharedListError: "Kunne ikke åpne den delte listen nå. Prøv igjen om litt.",
        ListNameRequired: "Skriv et navn på listen.",
        ListNameTooLong: "Navnet kan være høyst 200 tegn.",
        ListNameTaken: "Du har allerede en liste med dette navnet. Velg et annet navn.",
        CopyList: "Kopier liste",
        CopyListName: "Navn på kopien",
        CopyListSubmit: "Kopier listen",
        DefaultCopyName: name => $"{name} - kopi",
        CopyListLeavesDesiredData: "Kopien får variablene, men ikke det som står under Ønskede data.",
        CopyIncomplete: "Kopien er ufullstendig: ikke alle variablene ble kopiert. Listen under viser det som kom med.",
        CopyIncompleteElsewhere: name => $"Kopien «{name}» er ufullstendig: ikke alle variablene ble kopiert.",
        EmptyListAction: "Tøm liste",
        ConfirmEmptyList: "Fjerne alle variabler fra denne listen? Det kan ikke angres.",
        ConfirmEmptyYes: "Ja, tøm listen",
        ConfirmEmptyNo: "Nei",
    ...
    private static readonly Texts En = new(
        ...
        StatisticsFromOlderYearSet: yearSet => $"The statistics are from year set {yearSet}, the latest with any recorded.",
        ...
        ListActionError: "Could not change the list just now. Try again shortly.",
        ShareList: "Share list",
        ListIsEmpty: "The list is empty",
        ShareCodeLabel: "Share code",
        ShareLinkLabel: "Link to the list",
        ShareByEmail: "Send the code by email",
        ShareMailSubject: name => $"Shared variable list: {name}",
        ShareMailBody: (code, link) => link is null
            ? $"Open the list with the share code {code}. The code works for 90 days."
            : $"Open the list with the share code {code}, or with this link: {link} The code works for 90 days.",
        ShareValidity: "The code works for 90 days.",
        ShareError: "Could not share the list just now. Try again shortly.",
        OpenSharedList: "Open shared list",
        SharedListCodeLabel: "Share code of the list to open",
        OpenSharedListSubmit: "Open the list",
        SharedListEyebrow: "Shared list",
        SaveSharedList: "Save as my list",
        SaveSharedListName: "Name of your copy of the list",
        SaveSharedListSubmit: "Save the list",
        CloseSharedList: "Close shared list",
        SignInToSaveSharedList: "Sign in to save the list as your own.",
        SharedListNotFound: code => $"No shared list was found with the code {code}. A code works for 90 days.",
        SharedListError: "Could not open the shared list just now. Try again shortly.",
        ListNameRequired: "Enter a name for the list.",
        ListNameTooLong: "The name can be at most 200 characters.",
        ListNameTaken: "You already have a list with this name. Choose another name.",
        CopyList: "Copy list",
        CopyListName: "Name of the copy",
        CopyListSubmit: "Copy the list",
        DefaultCopyName: name => $"{name} - copy",
        CopyListLeavesDesiredData: "The copy gets the variables, but not what is written under Desired data.",
        CopyIncomplete: "The copy is incomplete: not every variable was copied. The list below shows what was.",
        CopyIncompleteElsewhere: name => $"The copy “{name}” is incomplete: not every variable was copied.",
        EmptyListAction: "Empty list",
        ConfirmEmptyList: "Remove every variable from this list? It cannot be undone.",
        ConfirmEmptyYes: "Yes, empty the list",
        ConfirmEmptyNo: "No",
