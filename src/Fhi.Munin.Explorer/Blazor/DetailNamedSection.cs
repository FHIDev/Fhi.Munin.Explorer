...
/// <c>metadata-</c>, which <see cref="DatasamlingView"/> finishes with the catalogue's own section
/// keys, so that set is Munin's to extend rather than a list this package can close. A repeated id
/// puts two elements on the page under it, and the nav's second link lands on the first. Keep it the
/// same in every language, since a reader shares the link.
/// </para>
/// </remarks>
