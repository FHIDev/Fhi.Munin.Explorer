...
/// <remarks>
/// <para>
/// <b>The compose-your-own route rather than the mount most hosts want.</b> It does nothing with
/// the address bar: it never reads <c>?kilde=</c> when it opens and never writes it when the reader
/// opens or closes one, so a link to this mount always lands on the list. Mounting it means owning
/// that yourself — read the query, pass <see cref="SelectedKildeId"/>, handle
/// <see cref="SelectedKildeIdChanged"/>, write back with <c>replaceState</c>.
/// <see cref="KildeExplorer"/> is that work already done, and is what a host mounts unless it wants
...
public sealed partial class KildeSearch : ComponentBase
{
    ...
    /// <c>KildeSearch.Selection.cs</c> for the whole of the reasoning, and both sample hosts for
    /// it written out.
    /// </para>
    /// <para>
    ...
    /// <see cref="KildeExplorer"/> declares and forwards it too, so either mount can
    ...
    // of the facets draw their choices out of it — see KildeSearch.Filters.cs — and it stays
