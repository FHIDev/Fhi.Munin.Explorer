...
public class ChangelogFragmentGuardTest
{
        ...
        IReadOnlyDictionary<string, string>? fragments = null)
    {
        ...
        CommitBranch(repo, changed, fragments);

        return Guard.RunIn(Script, repo.Path, "HEAD~1", "HEAD");
    }

    /// <summary>A throwaway repository holding the base commit this script diffs against.</summary>
    private static GuardRepository BaseRepository()
    {
        var repo = new GuardRepository("munin-changelog-guard");
