...
internal static class HostClassNames
{
    ...
    internal const string KilderSelectConsequence =
        ...
        + "UTC; 0.1.67 was already the newest on the feed before that merge, so 0.1.68 is the "
        + "first release carrying them, and the version helsedata.no pins. Get the signal wrong in either direction — the class missing where the handover is "
