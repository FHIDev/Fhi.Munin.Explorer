...
public class VariableSearchTest : ExplorerTestContext
{
    ...
    public void ExpandIcon_WhenASampleStandsInForStiler_ThenItSwapsImagesAndTurnsNothing()
    {
        ...
        // One image per direction, in the pairing the component now emits. `-down` is the collapsed
        // row, and the rule that says so is the one scoped to [aria-expanded=false]: the unscoped
        // `-down` override beside it still means "expanded", until step three takes it away.
