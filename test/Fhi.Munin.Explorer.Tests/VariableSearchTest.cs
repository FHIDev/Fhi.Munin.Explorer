...
public class VariableSearchTest : ExplorerTestContext
{
    ...
    private IRenderedComponent<VariableSearch> RenderWith(
        IMuninExplorerClient client, Action<ComponentParameterCollectionBuilder<VariableSearch>>? p = null)
    {
    ...
    public void Render_WhenTwoInstancesShareAPage_ThenTheirDomIdsDoNotCollide()
    {
        ...
        var a = Render<VariableSearch>();
        var b = Render<VariableSearch>();
    ...
    private static string CellText(IRenderedComponent<VariableSearch> cut, string key) =>
    ...
    public void Table_WhenTheSearchHasHits_ThenTheResultListExposesRowsAndColumns()
    {
        ...
        // Navn and the five optional columns that start on — Kode and Status start off. No
        // chevron column: the chevron is inside the name button (Fhi.Metadata-35w0p.78).
        ...
        // The five optional columns: one cell per header after the name's.
    ...
    public void Render_WhenTheHeadingLevelIsOutOfRange_ThenItIsClampedTo1Through6()
    {
        ...
        var low = Render<VariableSearch>(b => b.Add(c => c.HeadingLevel, 0));
        var high = Render<VariableSearch>(b => b.Add(c => c.HeadingLevel, 9));
    ...
    private static AngleSharp.Dom.IElement Previous(IRenderedComponent<VariableSearch> cut) =>
        PagerButtons(cut)[0];

    private static AngleSharp.Dom.IElement Next(IRenderedComponent<VariableSearch> cut) =>
        PagerButtons(cut)[^1];
    ...
    private static string StatusLine(IRenderedComponent<VariableSearch> cut) =>
        cut.Find("p[role='status']").TextContent;
    ...
    public void ExpandIcon_WhenASampleStandsInForStiler_ThenItSwapsImagesAndTurnsNothing()
    {
        ...
        // Collapsed writes `-down` and open writes `-up`, and since Stiler 0.1.110 the base icon
        // rules draw both at rest; the explorer's rules carry only the blue hover pair. An unscoped
        // `-down` mapped to `icon_up.svg` is the inverted override trfs0 removed (Fhi.Metadata-trfs0).
        Assert.DoesNotContain(rules, r => r.Selector.Contains("icon-keyboard-arrow-down", StringComparison.Ordinal)
                                          && Squeezed(r.Declarations).Contains("icon_up", StringComparison.Ordinal));

        Assert.Contains(HostClassNames.SampleDeclarationsFor("icon-keyboard-arrow-down"),
            r => r.Selector.Trim() == ".icon-keyboard-arrow-down"
                 && Squeezed(r.Declarations).Contains("icon_down.svg", StringComparison.Ordinal));

        Assert.Contains(HostClassNames.SampleDeclarationsFor("icon-keyboard-arrow-up"),
            r => r.Selector.Trim() == ".icon-keyboard-arrow-up"
    ...
    /// <summary>The glyphs one facet row draws, in the order it draws them.</summary>
    /// <remarks>
    /// Not "the datakategori glyphs": the kilde levels wear the grouping folder, which is
    /// deliberately no datakategori at all — <see cref="NodeIcons.SpokenCategories"/> withholds it.
    /// </remarks>
    ...
    public void Render_WhenADatasamlingCarriesNoCategories_ThenNoSlotIsDrawnAtAll()
    {
        ...
        var facets = FacetsWithCategories([], []);
        var cut = RenderWith(new FilteringClient(OnePage(), facets));

        // Every datasamling the fixture holds rather than two by name, so one added to it cannot
        // quietly fall out of this test's reach.
        Assert.All(facets.Datasamlinger,
                   datasamling => Assert.Empty(
                       Facet(cut, datasamling.Name).QuerySelectorAll(".munin-explorer-filters__icons")));
    ...
    [Fact]
    public void Render_WhenARowIsAHeadingRatherThanAChoice_ThenItDrawsNoDecorativeSlotAtAll()
    {
        // A heading is neither a kilde nor a delkilde and cannot be chosen, so a folder on it would
        // say it was one. What keeps that true whatever a FacetValue carries is that the slot is
        // written inside the label a checkbox opens, and this goes red if it moves out of there.
        var cut = RenderWith(new FilteringClient(OnePage(), FacetsWithBadges()));

        ExpandBranches(cut);

        var headings = FilterPanel(cut)
            .QuerySelectorAll("li")
            .Where(row => !row.Children.Any(child => child.TagName == "LABEL"))
            .ToList();

        // Or the sweep below passes over an empty panel: the fixture's three kildetyper are three
        // headings, and the rows beside them are drawing folders in the same render.
        Assert.NotEmpty(headings);
        Assert.Equal(["kilde"], RowGlyphs(cut, "Tromsøundersøkelsen"));
        Assert.All(headings,
                   heading => Assert.DoesNotContain(
                       heading.Children,
                       child => child.ClassList.Contains("munin-explorer-filters__icons")));
    }
    ...
    [InlineData("Biobank")]
    [InlineData("BIOBANK")]
    public void Render_WhenThePayloadSpellsABadgedKildetypeItsOwnWay_ThenTheBadgeIsStillDrawn(
        string spelling)
    {
        // Every kildetype table in this package is OrdinalIgnoreCase, because the value arrives as
        // the API's enum spelling or as a legacy stored one echoed back — so a badge lookup turned
        // ordinal by a refactor of the table would take the word off a payload that still sends it.
        var facets = FacetsWithBadges();
        var cut = RenderWith(new FilteringClient(OnePage(), facets with
        {
            Kilder = [.. facets.Kilder.Select(
                kilde => kilde.Id == Tromso ? kilde with { KildeType = spelling } : kilde)]
        }));
    ...
    /// <remarks>
    /// Rows rather than controls, because <see cref="Facet"/> reads a single one: a group hangs
    /// under every datasamling its variables are in, and the standalone facet may offer it too.
    /// </remarks>
    ...
    public void Variabelgrupper_WhenOneIsOptedOutOfTheStandaloneFacet_ThenTheKildeTreeStillDrawsIt()
    {
        ...
        var facets = FacetsWithDatasamlinger() with
        {
            HierarchyVariabelgrupper =
            [
                Variabelgruppe(Nutrition, "Kosthold", [Under(Tromso, datasamling: Tromso1)],
                       filter: "1", count: 5),
        ...
        // Each with the count the answer gave it: the opt-out decides the standalone facet's
        // membership and nothing about the row this tree draws.
        Assert.Equal(["Kosthold (5)", "Måltider (3)", "Miljø (2)"],
    ...
    public void Variabelgrupper_WhenEveryBranchIsOpened_ThenNoFilterMovedAndNothingWasFetched()
    {
        ...
        // The groups are offered as filters, and opening a branch ticks none of them: the checkbox
        // is drawn by the same press that drew the row, so a disclosure writing a filter would
        // narrow the results for a reader who only looked. (Fhi.Metadata-km3zb)
        var box = Assert.Single(Assert.Single(FilterPanelRows(cut, "Måltider")).QuerySelectorAll("input"));
    ...
    [Fact]
    public void Variabelgrupper_WhenOneIsAlreadyChosen_ThenItsRowInTheKildeTreeIsATickedCheckbox()
    {
        // FacetList draws a count and a marking inside the checkbox alone, so a row that had none
        // showed a chosen group as unchosen — and the count beside it says what the tick leaves.
        ...
        Assert.Equal("Kosthold (5)", RowWords(row));
    ...
    /// <summary>K_KK after a curator moved Cancer to the top, every child straight off the kilde.</summary>
    ...
    private static FilterOptions RankedKildeKk(int countOffset = 0)
    {
        ...
        return new FilterOptions
        {
            ...
            Kilder = [new() { Id = kilde, Name = "K_KK", KildeType = "biobank", Count = 20 + countOffset }],
            Delkilder =
            [
                .. scope.Delkilder.Select(wave => new DelkildeFacet
                {
                    Id = wave.Id, Name = wave.Name, KildeId = kilde, Count = ++count,
            ...
            [
                ...
                {
                    Id = datasamling.Id, Name = datasamling.Name, KildeId = kilde, Count = ++count,
    ...
    [Fact]
    public void KildeTree_WhenFiltersRankADatasamlingAheadOfTheWaves_ThenK_KKIsDrawnInTheResolvedOrder()
    {
        // Builder order proves nothing about the page if the panel regroups by kind on the way out;
        // Cancer ranks ahead of every wave, so a kind-first regroup fails here.
        var cut = RenderWith(new FilteringClient(OnePage(), RankedKildeKk()));

        Assert.Equal(SiblingOrderFixtures.ManualCancerFirst().Expected, KildeChildNames(cut, "K_KK"));
    }
    ...
    /// <remarks>
    /// Both surfaces draw a row per chosen group, so every assertion below says which one it read.
    /// A <c>&lt;details&gt;</c> renders its contents whether or not it is open, so this needs no press.
    /// </remarks>
    private static IElement StandaloneFacet(IRenderedComponent<VariableSearch> cut) =>
        Disclosures(cut)
            .Single(facet => facet.FirstElementChild!.TextContent.Trim()
                                  .StartsWith("Variabelgruppe", StringComparison.Ordinal));
    ...
    [Fact]
    public void Variabelgrupper_WhenOneIsTickedInTheStandaloneFacet_ThenTheKildeTreeTicksWithIt()
    {
        // The same claim from the other side, because either surface alone passes against the
        // other's failure: one writing a selection the other cannot read is still two selections.
        var client = new FilteringClient(OnePage(), FacetsWithBothSurfaces());
        var cut = RenderWith(client);
        ...
        Assert.Equal([Nutrition], client.SearchFilter!.VariabelgruppeIds);
        Assert.True(SurfaceBox(KildeFacet(cut), "Kosthold").HasAttribute("checked"));
        Assert.Equal(["Kosthold"], Chips(cut));
    ...
    [Fact]
    public void Variabelgrupper_WhenAnOptedOutGroupIsTickedInTheTree_ThenItNeverReachesTheOtherFacet()
    {
        // The opt-out is the standalone facet's rule, and selecting a group is not a way round it:
        // a ticked id appearing there would offer the reader a filter the API says they may not
        // have. The tree is the one control over it, and stays one. (Fhi.Metadata-fbe3w)
        var client = new FilteringClient(OnePage(), FacetsWithBothSurfaces());
        var cut = RenderWith(client);
    ...
    [Fact]
    public void Variabelgrupper_WhenADeepLinkNamesAnOptedOutGroup_ThenTheTreeTicksItAndTheFacetOmitsIt()
    {
        // A shared link carries ids and knows nothing of the opt-out, so restoring one has to reach
        // the surface that offers it — and the trail has to name it out of the tree's own
        // collection, the facet list it is absent from having been the only lookup.
        var client = new FilteringClient(OnePage(), FacetsWithBothSurfaces());
        var cut = RenderFiltered(client, new VariableFilter { VariabelgruppeIds = [Meals] });
        ...
        Assert.Equal(["Måltider"], Chips(cut));
        Assert.Contains("Variabelgruppe (1)", FacetHeadings(cut));
        Assert.Equal("Måltider", Crumbs(cut)[^1].TextContent);
    }

    [Fact]
    public void Variabelgrupper_WhenAnOptedOutGroupsChipIsRemoved_ThenTheKildeTreeCheckboxClears()
    {
        // The chip and the tree's checkbox are two controls over one id, so a removal that left the
        // box ticked would show a filter still in force that nothing was narrowing by.
        var client = new FilteringClient(OnePage(), FacetsWithBothSurfaces());
        var cut = RenderFiltered(client, new VariableFilter { VariabelgruppeIds = [Meals] });

        ExpandBranches(cut);
        RemoveChip(cut, "Måltider");

        Assert.Empty(client.SearchFilter!.VariabelgruppeIds);
        Assert.Empty(Chips(cut));
        Assert.False(SurfaceBox(KildeFacet(cut), "Måltider").HasAttribute("checked"));
    }

    [Fact]
    public void Variabelgrupper_WhenABranchIsShutAndReopened_ThenTheSelectionAndItsOneChipStand()
    {
        // Expansion is the panel's own state and selection is the filter's: a fold that dropped a
        // tick would take a filter off with no control having said so, and the reader's only sign
        // of it would be the result count moving.
        var client = new FilteringClient(OnePage(), FacetsWithBothSurfaces());
        var cut = RenderWith(client);
    ...
    private static IElement Crumb(IRenderedComponent<VariableSearch> cut, string label) =>
    ...
    private IRenderedComponent<VariableSearch> RenderFiltered(
    ...
    public void Panel_WhenTheDataTabIsChosen_ThenTheStatisticsShowBesideTheKodeverk()
    {
        ...
        // The numbers themselves, not just their heading. The drawer no longer draws the table;
        // the whole-variable view does (Fhi.Metadata-9mxmw).
        Assert.NotNull(panel.QuerySelector("dl.munin-explorer-figures"));
    ...
    public void Panel_WhenTheVariableHasNoStatistics_ThenTheDataTabDrawsNeitherHeadingNorTable()
    {
        ...
        Assert.Empty(panel.QuerySelectorAll(
            ".munin-explorer-coverage, .munin-explorer-distribution, .munin-explorer-figures, .munin-explorer-figures__note"));
    }
    ...
    // ---------------------------------------------------------------------------------
    // The Data tab's round-5 statistics: a coverage line, a bar per code and the figures, with the
    // catalogue's own wording where Munin marks part of a statistic as withheld (Fhi.Metadata-9mxmw,
    // Fhi.Metadata-35w0p.36). The whole-variable view keeps the table; VariableViewTest pins that.

    private static Statistic DataTabStatistic(
        Dictionary<string, string?> properties,
        IReadOnlyList<CodeFrequency>? frequencies = null,
        string? disclosureControl = null) =>
        new()
        {
            AdditionalProperties = properties,
            CodeFrequencies = frequencies ?? [],
            DisclosureControl = disclosureControl,
        };

    private static CodeFrequency CodeCount(string localId, string term, string? count) =>
        new()
        {
    ...
    private static Dictionary<string, string?> Counts(string? valid, string? missing)
    {
        var properties = new Dictionary<string, string?> { ["SisteOppdaterteAarssett"] = "2026" };

        if (valid is not null)
        {
            properties["GyldigeTilfeller"] = valid;
        }

        if (missing is not null)
        {
            properties["ManglendeTilfeller"] = missing;
        }

        return properties;
    }

    private IElement DataTab(
        IReadOnlyList<Statistic> statistics,
        string? language = null,
        string statisticsType = "Accumulated",
        bool kodeverk = true)
    {
        var detail = Detail(TaleId) with
        {
            DatasamlingStatisticsType = statisticsType,
            Statistics = statistics,
            KodeverkLinks = kodeverk ? Detail(TaleId).KodeverkLinks : [],
        };
        var cut = OpenData(
            new DetailClient(OnePage(Row(TaleId, "1. Tale"))).Knows(detail),
            language is null ? null : p => p.Add(c => c.Language, language));

        return Panel(cut).QuerySelector("[role=tabpanel]")!;
    }

    private static string Formatted(long count, string language) =>
        count.ToString("N0", CatalogueProperties.Culture(language));

    [Theory]
    [InlineData("no", "gyldige", "mangler", "82 % dekning")]
    [InlineData("en", "valid", "missing", "82% coverage")]
    public void DataTab_WhenBothCountsArePresent_ThenTheCoverageLineStatesThemInTheReadersFormat(
        string language, string validWord, string missingWord, string share)
    {
        var tab = DataTab([DataTabStatistic(Counts("15578", "3461"))], language);
        var valid = Formatted(15578, language);
        var missing = Formatted(3461, language);

        var coverage = tab.QuerySelector("div.munin-explorer-coverage")!;
        var line = coverage.QuerySelector(".munin-explorer-coverage__line")!;

        // Only the number inside __valid: the mockup bolds the number and not the word after it.
        Assert.Equal(valid, line.QuerySelector(".munin-explorer-coverage__valid")!.TextContent);
        Assert.StartsWith($"{valid} {validWord}", line.TextContent, StringComparison.Ordinal);
        Assert.Equal($"{missing} {missingWord}", line.QuerySelector(".munin-explorer-coverage__missing")!.TextContent);
        Assert.Equal(share, line.QuerySelector(".munin-explorer-coverage__share")!.TextContent);

        var bar = coverage.QuerySelector(".munin-explorer-coverage__bar")!;
        var label = bar.GetAttribute("aria-label")!;

        Assert.Equal("img", bar.GetAttribute("role"));
        Assert.Contains(valid, label, StringComparison.Ordinal);
        Assert.Contains(missing, label, StringComparison.Ordinal);
        Assert.Contains("82", label, StringComparison.Ordinal);
        Assert.Equal("width:82%", bar.QuerySelector(".munin-explorer-coverage__bar-fill")!.GetAttribute("style"));
    }
    ...
    [InlineData("5", "0", 100)]
    [InlineData("0", "5", 0)]
    public void DataTab_WhenCoverageIsWorkedOut_ThenItIsValidOverValidPlusMissingToAWholePercent(
        string valid, string missing, int expected)
    {
        // Over valid plus missing, never over the frequencies: those sum to the valid cases alone.
        var tab = DataTab([DataTabStatistic(Counts(valid, missing), [CodeCount("0", "Ja", "999")])]);

        Assert.Equal($"{expected} % dekning", tab.QuerySelector(".munin-explorer-coverage__share")!.TextContent);
        Assert.Equal($"width:{expected}%",
                     tab.QuerySelector(".munin-explorer-coverage__bar-fill")!.GetAttribute("style"));
    }

    [Theory]
    [InlineData("5470", null)]
    [InlineData(null, "18461")]
    [InlineData(null, null)]
    public void DataTab_WhenEitherCountIsMissing_ThenNoCoverageLineIsDrawnEvenWithFrequencies(
        string? valid, string? missing)
    {
        // Summing the frequencies would give the valid cases and never the total, so a guess from
        // them reads 100 % on every variable; with a count missing there is nothing to draw.
        var tab = DataTab([DataTabStatistic(
            Counts(valid, missing), [CodeCount("0", "Ja", "1769"), CodeCount("1", "Nei", "2651")])]);

        Assert.NotNull(tab.QuerySelector("ul.munin-explorer-distribution"));
        Assert.Empty(tab.QuerySelectorAll(".munin-explorer-coverage"));
        Assert.DoesNotContain("dekning", tab.TextContent, StringComparison.Ordinal);
    }

    [Fact]
    public void DataTab_WhenFrequenciesArePresent_ThenEachCodeIsABarInTheApisOrderWithItsCountAsText()
    {
        // Out of count order on purpose: the order is Munin's display order, and a sort by size
        // here would be this package deciding it.
        var tab = DataTab([DataTabStatistic(
            Counts("5470", "18461"),
            [CodeCount("2", "Delvis", "920"), CodeCount("0", "Ja", "1769"), CodeCount("7", "", "2651")])]);

        var items = tab.QuerySelectorAll("ul.munin-explorer-distribution > li.munin-explorer-distribution__item");

        Assert.Equal(["Delvis", "Ja", "7"],
                     items.Select(item => item.QuerySelector(".munin-explorer-distribution__label")!.TextContent));
        Assert.Equal([Formatted(920, "no"), Formatted(1769, "no"), Formatted(2651, "no")],
                     items.Select(item => item.QuerySelector(".munin-explorer-distribution__count")!.TextContent));
        Assert.DoesNotContain("449", tab.QuerySelector("ul.munin-explorer-distribution")!.TextContent, StringComparison.Ordinal);

        // Decorative: the fill is 1.13:1 against the drawer, so the count text is the information.
        Assert.All(items, item => Assert.Equal(
            "true", item.QuerySelector(".munin-explorer-distribution__bar")!.GetAttribute("aria-hidden")));

        // Relative to the largest count, not to the valid cases: 2651 is the full bar.
        Assert.Equal(["width:34.7%", "width:66.7%", "width:100%"],
                     items.Select(item => item.QuerySelector(".munin-explorer-distribution__bar-fill")!.GetAttribute("style")));
    }

    [Theory]
    [InlineData("no", false, new[] { "Min", "Maks", "Snitt" }, new[] { "1", "9", "4.2" })]
    [InlineData("no", true, new[] { "Min", "Median", "Maks", "Snitt" }, new[] { "1", "3", "9", "4.2" })]
    [InlineData("en", true, new[] { "Min", "Median", "Max", "Mean" }, new[] { "1", "3", "9", "4.2" })]
    public void DataTab_WhenSomeFiguresArePresent_ThenOnlyThoseAreListedInOrder(
        string language, bool median, string[] terms, string[] values)
    {
        var properties = new Dictionary<string, string?> { ["MIN"] = "1", ["MAX"] = "9", ["AVG"] = "4.2", ["STD"] = "2" };
        ...
        var figures = DataTab([DataTabStatistic(properties)], language).QuerySelector("dl.munin-explorer-figures")!;
        var items = figures.QuerySelectorAll("div.munin-explorer-figures__item");

        Assert.Equal(terms, items.Select(item => item.QuerySelector("dt.munin-explorer-figures__term")!.TextContent));
        Assert.Equal(values, items.Select(item => item.QuerySelector("dd.munin-explorer-figures__value")!.TextContent));
    }

    [Theory]
    [InlineData(Statistic.DescriptiveStatisticsNotGiven, "no",
                "Statistisk avsløringskontroll: deskriptiv statistikk ikke gitt")]
    [InlineData(Statistic.DescriptiveStatisticsNotGiven, "en",
                "Statistical disclosure control: descriptive statistics not given")]
    [InlineData(Statistic.CategoriesNotSummarised, "no",
                "Statistisk avsløringskontroll: kategorier ikke oppsummert")]
    [InlineData(Statistic.CategoriesNotSummarised, "en",
                "Statistical disclosure control: categories not summarised")]
    public void DataTab_WhenPartOfAStatisticIsWithheld_ThenThatPartIsTheCataloguesNoteUnderTheCoverageLine(
        string control, string language, string wording)
    {
        var properties = Counts("5470", "18461");
        properties["MIN"] = "1";
        var tab = DataTab(
            [DataTabStatistic(properties, [CodeCount("0", "Ja", "1769")], control)], language);

        var note = Assert.Single(tab.QuerySelectorAll("p.munin-explorer-figures__note"));

        Assert.Equal(wording, note.TextContent);
        Assert.Contains("munin-explorer-figures__note--suppressed", note.ClassList);

        // Only the part the marker names is replaced; the other is still drawn.
        var descriptive = control == Statistic.DescriptiveStatisticsNotGiven;

        Assert.Equal(descriptive, tab.QuerySelector("dl.munin-explorer-figures") is null);
        Assert.Equal(descriptive, tab.QuerySelector("ul.munin-explorer-distribution") is not null);

        // And the coverage line stays drawn, above the note.
        var inOrder = tab.QuerySelectorAll(".munin-explorer-coverage, .munin-explorer-figures__note");

        Assert.Equal(2, inOrder.Length);
        Assert.Contains("munin-explorer-coverage", inOrder[0].ClassList);
        Assert.Same(note, inOrder[1]);
    }

    [Fact]
    public void DataTab_WhenAStatisticHasNoValuesAndNoMarker_ThenItReadsAsAbsentAndNeverAsWithheld()
    {
        // THE FALSE CLAIM. "Withheld for disclosure control" over a statistic nobody computed says
        // FHI holds data it may not hold, on the page researchers use to decide what to ask for. A
        // bare year set is the shape: it used to draw a heading over dashes (Fhi.Metadata-35w0p.36).
        var id = Guid.NewGuid();
        var variable = Detail(id) with
        {
            KodeverkLinks = [],
            DatasamlingStatisticsType = "yearly",
            Statistics = [DataTabStatistic(new Dictionary<string, string?> { ["SisteOppdaterteAarssett"] = "2022" })],
        };
        var cut = OpenData(new DetailClient(OnePage(Row(id, "1. Tale"))).Knows(variable));
        var tab = Panel(cut).QuerySelector("[role=tabpanel]")!;

        var absent = Assert.Single(tab.QuerySelectorAll($"p.caption.{DetailBlocks.Absent}"));

        Assert.Equal("Ingen kodeverk eller statistikk registrert", absent.TextContent);
        Assert.DoesNotContain("avsløringskontroll", tab.TextContent, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Statistikk (", tab.TextContent, StringComparison.Ordinal);
        Assert.Empty(tab.QuerySelectorAll(".munin-explorer-figures__note--suppressed"));

        // One decision point, so the whole variable agrees that there is nothing to show.
    ...
    [Fact]
    public void DataTab_WhenAStatisticHasOnlyAMarker_ThenTheNoteIsDrawnRatherThanTheAbsentLine()
    {
        ...
        Assert.Contains("Statistikk (akkumulert)", tab.TextContent, StringComparison.Ordinal);
        Assert.Equal("Statistisk avsløringskontroll: deskriptiv statistikk ikke gitt",
                     tab.QuerySelector(".munin-explorer-figures__note--suppressed")!.TextContent);
    }
    ...
    public void DataTab_WhenAYearlyVariableHasSeveralStatistics_ThenTheNewestWithNumbersIsDrawn()
    {
        ...
        Statistic Year(string year, string? minimum) => DataTabStatistic(minimum is null
            ...
            : new Dictionary<string, string?> { ["SisteOppdaterteAarssett"] = year, ["MIN"] = minimum });

        var tab = DataTab([Year("2021", "1"), Year("2023", "5"), Year("2022", "3"), Year("2024", null)],
                          statisticsType: "yearly");

        var figures = Assert.Single(tab.QuerySelectorAll("dl.munin-explorer-figures"));
    ...
    private static IReadOnlyList<string> KodeverkGroupHeadings(IRenderedComponent<VariableSearch> cut) =>
        ...
        bool kodeverk, bool statistics)
    {
        ...
        var tabStatistics = tab.QuerySelector("dl.munin-explorer-figures") is not null;
    ...
    [Fact]
    public void Detail_WhenTheChevronIsDrawn_ThenItsGlyphIsTheNameButtonsFirstChildAndNowhereElse()
    {
        // The trap zqe14 closed still applies: a click handler on the aria-hidden glyph would give
        // assistive tech one control and a mouse two. The glyph is the name button's first child,
        // and there is no other in the row. (Fhi.Metadata-35w0p.78)
        var cut = RenderWith(TwoRows());
        var toggle = Toggles(cut)[0];
        var chevron = toggle.QuerySelector(".munin-explorer-dataitem-main__expand-icon");

        Assert.NotNull(chevron);
        Assert.Same(chevron, toggle.FirstElementChild);
        Assert.Equal("true", chevron!.GetAttribute("aria-hidden"));
        Assert.Equal("false", toggle.GetAttribute("aria-expanded"));
        Assert.Equal("1. Tale", toggle.TextContent.Trim());
        Assert.Single(cut.FindAll("li.munin-explorer-data-list__item")[0].QuerySelectorAll(".munin-explorer-dataitem-main__expand-icon"));
    }
    ...
    /// </remarks>
    private static IReadOnlyList<AngleSharp.Dom.IElement> SourceToggles(IRenderedComponent<VariableSearch> cut) =>
    ...
    {
        // The guard the chevron, the row name and "Vis koder" carry, on a disclosure that was still
        // built from a parameterless lambda: unguarded, the second click of a double-click reached
        // ToggleSourceAsync and shut the owner the first click had just opened. (Fhi.Metadata-j1j3i)
        var client = TwoRows();
        var cut = RenderWith(client);
    ...
    [Fact]
    public void SourceToggle_WhenEachPressIsItsOwnGesture_ThenTheOwnerOpensEveryTime()
    {
        // The guard is per gesture, not per control: a reader leaves the view by its own way back,
        // so the next press of this button is a fresh gesture and has to open the kilde again. The
        // second one reports no click count at all, which is how a browser sends Enter and Space.
        var client = TwoRows();
        var cut = RenderWith(client);

        Toggles(cut)[0].Click();
        PressOwnerControl(SourceToggles(cut)[0]);
    ...
    private static AngleSharp.Dom.ITokenList BackClasses(IRenderedComponent<VariableSearch> cut) =>
    ...
    /// </remarks>
    private static IReadOnlyList<IElement> RetryButtons(IRenderedComponent<VariableSearch> cut) =>
        cut.FindAll("div[role='alert'][aria-live='assertive'] button");

    private static IElement Retry(IRenderedComponent<VariableSearch> cut, string label) =>
    ...
    /// (<c>VariableSearch.SaveButton.cs</c>), and an unscoped selector would start counting those.
    /// </remarks>
    private static IReadOnlyList<IElement> AlertMessages(IRenderedComponent<VariableSearch> cut) =>
